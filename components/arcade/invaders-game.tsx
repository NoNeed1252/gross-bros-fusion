...
function resolveWallet(explicit?: string): string {
  if (explicit && explicit.length > 8) return explicit
  if (typeof window === "undefined") return "Anonymous"
  const fromStorage =
    localStorage.getItem("wallet_address") ||
    localStorage.getItem("xaman_wallet") ||
    ""
  if (fromStorage && fromStorage.length > 8) return fromStorage
  return "Anonymous"
}
...
export function InvadersGame({
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, s: GameState | null) => {
    ...
    for (const b of s.barriers) {
      ...
      if (typeof (ctx as any).roundRect === "function")
        ctx.roundRect(b.x, b.y, BARRIER_W, BARRIER_H, 4)
    ...
    for (const inv of s.invaders)
      if (inv.alive) drawEnemy(ctx, inv.x, inv.y, inv.type, stepCount)
    ...
    for (const pu of s.powerups) {
      ...
      ctx.fillText(
        pu.type[0].toUpperCase(),
        pu.x + POWERUP_SIZE / 2,
        pu.y + POWERUP_SIZE / 2 + 4
      )
    }
  ...
  const loop = useCallback(() => {
    ...
    if (statusRef.current === "playing" && s) {
      ...
      for (const pu of s.powerups) {
        pu.y += POWERUP_SPEED
        if (
          pu.x > s.playerX - POWERUP_SIZE &&
          pu.x < s.playerX + PLAYER_W &&
          pu.y > PLAYER_Y &&
          pu.y < PLAYER_Y + PLAYER_H
        ) {
  ...
  if (!mounted)
    return <div className="aspect-[640/560] w-full rounded-2xl bg-[#0a1512]" />
  ...
  return (
    <div className="select-none">
      <div className="relative overflow-hidden rounded-2xl border border-border/70 bg-[#0a1512] neon-border">
            ...
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-4 bg-background/80 px-6 text-center backdrop-blur-sm">
              ...
              {status === "over" ? (
                ...
                <p className="max-w-xs text-pretty text-sm leading-relaxed text-muted-foreground">
                  The Gross Bros broke loose. Pilot your Bro and defend the
                  Ledger.
                </p>
              )}
              <button
                type="button"
                onClick={startGame}
                className="neon-ring rounded-xl bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground transition-all hover:opacity-90"
              >
        ...
        {status === "playing" && (
          <div className="rounded-b-2xl border-t border-primary/30 bg-[#0a1512] px-4 py-3 md:hidden">
            <div className="flex items-center justify-between gap-3">
              <button
                ...
                className="flex h-12 w-12 select-none items-center justify-center rounded-xl border border-primary/40 bg-[#111f1a] text-3xl font-bold text-primary transition-all active:scale-[0.95] active:bg-primary/10"
              >
                &lt;
              </button>
              ...
              <button
                ...
                className="h-14 max-w-[210px] flex-1 select-none rounded-2xl bg-primary text-lg font-extrabold tracking-[3px] text-[#0a1512] shadow-[0_0_25px_#00ff9f] transition-all active:scale-[0.985] active:bg-[#00cc7a]"
              >
                FIRE
              </button>
